...
function ensureProject(recruiter, name) {
  const existing = db
    .prepare('SELECT * FROM projects WHERE org_id = ? AND name = ?')
    .get(recruiter.org_id, name);
  if (existing) return existing;

  const projectId = crypto.randomUUID();
  db.prepare('INSERT INTO projects (id, org_id, created_by, name, created_at) VALUES (?, ?, ?, ?, ?)')
    .run(projectId, recruiter.org_id, recruiter.id, name, Date.now());
...
for (const projSpec of SAMPLE_PROJECTS) {
  const project = ensureProject(recruiter, projSpec.name);
  console.log(`  Project: ${project.name}`);
  for (const spec of projSpec.forms) {
    const f = ensureForm(recruiter, project, spec);
