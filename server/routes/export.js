...
function collectExport(forms) {
  ...
  const getAnswers = db.prepare(`
    SELECT a.id, a.audio_path, a.transcript, a.transcript_status,
  ...
  for (const form of forms) {
    ...
    for (const r of responses) {
      ...
      const jsonAnswers = [];
      answers.forEach((a, i) => {
        const ext = path.extname(a.audio_path) || '.webm';
        const audioFile = `${respFolder}/Q${String(i + 1).padStart(2, '0')}${ext}`;
        rows.push([
          form.title, form.code, r.candidate_name,
          new Date(r.submitted_at).toISOString(),
          r.consent_given_at ? new Date(r.consent_given_at).toISOString() : '',
          r.consent_ip || '',
          i + 1, a.question_text, a.transcript || '', a.transcript_status || '',
          audioFile
        ]);
...
function streamZip(res, forms, downloadName) {
  ...
  const header = [
    'form_title', 'form_code', 'respondent', 'submitted_at', 'consent_given_at',
    'consent_ip', 'question_order', 'question_text', 'transcript', 'transcript_status', 'audio_file'
  ];
